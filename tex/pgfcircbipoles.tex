...
{\ctikzvalof{bipoles/oosourcetrans/width}}
{
    ...
    \ifpgf@circ@prim@delta
        \pgfscope
            \pgftransformxshift{\ctikzvalof{bipoles/oosourcetrans/vectorgroupoffset}\pgf@circ@res@left}
    ...
    \ifpgf@circ@sec@delta
        \pgfscope
            \pgftransformxshift{\ctikzvalof{bipoles/oosourcetrans/vectorgroupoffset}\pgf@circ@res@right}
            \pgf@circ@delta{\ctikzvalof{bipoles/oosourcetrans/vectorgroupscale}}
        \endpgfscope
...
% And the height is not *exactly* the height of the shape --- with default parameters
% the real height is 0.939 the "height" parameter (sin(60)+0.55(1-sin(60))
% that makes the geo-anchor slightly off, but...
\pgfcircdeclarebipolescaled{sources}
{
    \savedmacro{\circlesize}{\edef\circlesize{%
        \ctikzvalof{bipoles/ooosource/circlesize}}%
    }
    \savedmacro{\circleoffset}{%
        \pgfmathparse{1.0-\ctikzvalof{bipoles/ooosource/circlesize}}%
    ...
    \anchor{right}{
            \northeast
            \pgf@y=0pt
            \pgfmathparse{ \circleoffset* sin(30) +
            %%the sqrt must be > 0, the circles have to intersect
                sqrt(pow(\circlesize,2) - pow(\circleoffset*cos(30),2))
            }
    ...
    \savedanchor{\centerprim}{% remove me
            ...
            \pgf@circ@res@other=-\circleoffset\pgf@circ@res@other
            \pgf@y=0pt
            \pgf@x=.5\pgf@circ@res@other
    }
    \anchor{centerprim}{
        \northeast
        \pgf@x=\circleoffset\pgf@x\pgf@x=-\pgf@x
        \pgf@y=0pt
    }
    ...
    \anchor{prim1}{
            \pgf@circ@scaled@Rlen=\ctikzvalof{\ctikzclass/scale}\pgf@circ@Rlen
            \pgf@circ@res@other=\ctikzvalof{bipoles/ooosource/height}\pgf@circ@scaled@Rlen
            \pgf@circ@res@other=\circlesize\pgf@circ@res@other
    ...
    \anchor{symbolsec}{
        ...
        \pgf@ya=\pgf@y
        \pgf@xa=\pgf@x
        \pgfpointpolar{60}{\vectorgroupoffset\pgf@ya}
    }
    \anchor{symboltert}{
        \northeast
        \pgf@ya=\pgf@y
...
{
    % notice that all the length "res" are the same for now. To avoid confusion,
    % we use a different lenght:
    \pgf@circ@res@other=\pgf@circ@res@up
    % \typeout{SIZE,OFFSET:\circlesize,\circleoffset,\the\pgf@circ@res@up}
    % filling
    % left
    \pgf@circ@setlinewidth{bipoles}{\pgfstartlinewidth}
