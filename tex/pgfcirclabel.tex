...
\def\pgf@circ@drawrotlabels#1{
	\pgfextra{
                ...
                % the 1.2 is adjusted so that the distance is more or less
                ...
                \pgfmathsetlength\pgf@circ@res@temp{1.2*\pgf@circ@ls/\myscale}
		...
		%we need some more space for placement below, due to mid-anchor
			...
			\pgf@circ@res@temp=2\pgf@circ@res@temp
		\fi
...
\def\pgf@circ@drawreglabels#1{
	...
	%Values between -5 and 5 are at pos /neg x-axis
                ...
                \ifnum \pgfcircmathresult <6 \ifnum \pgfcircmathresult > -6
                    \ifnum \ctikzvalof{bipole/#1/position} < 0
                        \ifnum \pgf@circ@labanc > 90
                            ...
                            % reset cm is needed to avoid the scaling of ex (thanks Eric Marsden)
                            % see https://tex.stackexchange.com/a/476004/38080
                            {[reset cm](labelcoor) ++(-\pgf@circ@labanc:\pgf@circ@ls)} coordinate(labelcoor)
        ...
        (labelcoor) node[anchor=\pgf@circ@labanctext,
            inner sep=\pgf@circ@ls, outer sep=0pt,
