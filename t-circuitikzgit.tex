\def\pgfcircversion{git:f649de6}
\def\pgfcircversiondate{2017/03/07}
...
\ctikzset{bipoles/open/voltage/distance from node/.initial=.5}
...
%coordinate (bla) at (\tikztotarget)
%coordinate (start) at (\tikztostart)
%(\tikztotarget)
%(\tikztostart)
		...
		\pgfextra{
			...
			\pgf@circ@slope{\pgfpoint{\pgf@xa}{\pgf@ya}}{\pgfpoint{\pgf@xb}{\pgf@yb}}
			\edef\pgf@circ@direction{\pgfmathresult}
%\pgfpathmoveto{\pgfpoint{\pgf@xa}{\pgf@ya}}	
%\pgfpathlineto{\pgfpointlineatdistance{1mm}{\pgfpoint{\pgf@xa}{\pgf@ya}}{\pgfpoint{\pgf@xb}{\pgf@yb}}}
%\pgfusepath{draw}
		}


%--($(\tikztostart) ! .5\pgflinewidth ! (\tikztotarget)$) %ugly workaround to get correct linejoins(node breaks path?)
	...
	%($(\ctikzvalof{bipole/name}end) ! .5\pgflinewidth ! (\ctikzvalof{bipole/name}start)$) -- %ugly workaround to get correct linejoins(node breaks path?)
	%tikztostart and tikztotarget could not be used, because it would break if target coordinate is something like (node1-|node2)
(\ctikzvalof{bipole/name}end)
	% reset
	\pgfextra{
		\pgfcircresetpath
	}  
