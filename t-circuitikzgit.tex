\def\pgfcircversion{git:c4adc6f}
\def\pgfcircversiondate{2019/02/06}
...
\def\pgf@circ@drawreglabels#1{
	...
	\ifnum \pgfcircmathresult <6 \ifnum \pgfcircmathresult > -6
		\ifnum \ctikzvalof{bipole/#1/position} < 0
			%we need some more space for placement below, due to mid-anchor
			(labelcoor)++(-\pgf@circ@labanc:1ex) coordinate(labelcoor)
		...
		\fi
	\fi\fi
	 (labelcoor) node[anchor=\pgf@circ@labanctext,inner sep=0,outer sep=0](\ctikzvalof{bipole/name}#1){\pgf@circ@finallabels{#1}}
}
